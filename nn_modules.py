...
class FaceXZooProjector(nn.Module):
    ...
    def align_patch(self, adv_patch, landmarks):
        ...
        max_side_dist = torch.maximum(landmarks[:, 33, 0] - landmarks[:, 2, 0],
        ...
        left_top = torch.stack((landmarks[:, 33, 0] - max_side_dist, landmarks[:, 62, 1] - self.mask_half_height),
                               dim=-1)
        right_top = torch.stack((landmarks[:, 33, 0] + max_side_dist, landmarks[:, 62, 1] - self.mask_half_height),
                                dim=-1)
        left_bottom = torch.stack((landmarks[:, 33, 0] - max_side_dist, landmarks[:, 62, 1] + self.mask_half_height),
                                  dim=-1)
        right_bottom = torch.stack((landmarks[:, 33, 0] + max_side_dist, landmarks[:, 62, 1] + self.mask_half_height),
                                   dim=-1)
        dst_pts = torch.stack([left_top, right_top, left_bottom, right_bottom], dim=1)

        tform = kornia.find_homography_dlt(src_pts, dst_pts)
        cropped_image = kornia.geometry.warp_perspective(adv_patch, tform,
